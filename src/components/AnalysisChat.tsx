    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="max-h-96 overflow-y-auto space-y-3">
          {messages.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm mt-2">
                Examples: "What are the highest risk issues?" or "Explain the coordination conflicts"
              </p>
            </div>
          ) : (
              ...
              <div
                ...
                className={`p-3 rounded-lg ${
                  message.type === 'user'
                    ...
                    : 'bg-slate-50 border-l-4 border-slate-400 mr-8'
                }`}
              >
                <div className="flex justify-between items-start mb-1">
                  <span className="font-medium text-sm">
                    {message.type === 'user' ? 'You' : 'StudioCheck AI'}
                  </span>
          ...
          {isLoading && (
            <div className="bg-slate-50 border-l-4 border-slate-400 mr-8 p-3 rounded-lg">
              <div className="flex items-center gap-2">
                <Loader2 className="h-4 w-4 animate-spin" />
                <span className="text-sm text-slate-600">StudioCheck AI is thinking...</span>
              </div>
            </div>
          )}
        </div>
